...
# Script to send event reminders to all approved attendees
# Usage: 
#   ./send-event-reminder.sh                    # Send to all approved attendees
#   ./send-event-reminder.sh TEST_DISCORD_ID    # Send test to specific Discord ID
...
FUNCTION_URL="${NEXT_PUBLIC_SUPABASE_URL}/functions/v1/send-event-reminder"
...
if [ -n "$1" ]; then
    ...
    echo "📨 PRODUCTION MODE: Sending REMINDER to ALL approved attendees"
    read -p "Are you sure you want to send the REMINDER to ALL approved attendees? (yes/no): " CONFIRM
    if [ "$CONFIRM" != "yes" ]; then
        echo "Cancelled."
        exit 0
    fi
